...
// We need both the attribute and keyword to avoid "might not be inlineable"
// warnings.
...
static LIBDIVIDE_INLINE struct libdivide_s16_branchfree_t
libdivide_s16_branchfree_gen(int16_t d);
static LIBDIVIDE_INLINE struct libdivide_u16_branchfree_t
libdivide_u16_branchfree_gen(uint16_t d);
static LIBDIVIDE_INLINE struct libdivide_s32_branchfree_t
libdivide_s32_branchfree_gen(int32_t d);
static LIBDIVIDE_INLINE struct libdivide_u32_branchfree_t
libdivide_u32_branchfree_gen(uint32_t d);
static LIBDIVIDE_INLINE struct libdivide_s64_branchfree_t
libdivide_s64_branchfree_gen(int64_t d);
static LIBDIVIDE_INLINE struct libdivide_u64_branchfree_t
libdivide_u64_branchfree_gen(uint64_t d);

static LIBDIVIDE_INLINE int16_t libdivide_s16_do_raw(int16_t numer,
                                                     int16_t magic,
                                                     uint8_t more);
static LIBDIVIDE_INLINE int16_t
libdivide_s16_do(int16_t numer, const struct libdivide_s16_t *denom);
static LIBDIVIDE_INLINE uint16_t libdivide_u16_do_raw(uint16_t numer,
                                                      uint16_t magic,
                                                      uint8_t more);
static LIBDIVIDE_INLINE uint16_t
libdivide_u16_do(uint16_t numer, const struct libdivide_u16_t *denom);
static LIBDIVIDE_INLINE int32_t
...
static LIBDIVIDE_INLINE int16_t
libdivide_s16_recover(const struct libdivide_s16_t *denom);
static LIBDIVIDE_INLINE uint16_t
libdivide_u16_recover(const struct libdivide_u16_t *denom);
static LIBDIVIDE_INLINE int32_t
...
static LIBDIVIDE_INLINE uint16_t libdivide_32_div_16_to_16(uint16_t u1,
                                                           uint16_t u0,
                                                           uint16_t v,
                                                           uint16_t *r) {
...
// Returns the quotient directly and the remainder in *r
static LIBDIVIDE_INLINE uint32_t libdivide_64_div_32_to_32(uint32_t u1,
                                                           uint32_t u0,
                                                           uint32_t v,
                                                           uint32_t *r) {
...
// libdivide_128_div_64_to_64: divides a 128-bit uint {numhi, numlo} by a 64-bit
// uint {den}. The result must fit in 64 bits. Returns the quotient directly and
// the remainder in *r
static LIBDIVIDE_INLINE uint64_t libdivide_128_div_64_to_64(uint64_t numhi,
                                                            uint64_t numlo,
                                                            uint64_t den,
                                                            uint64_t *r) {
    // N.B. resist the temptation to use __uint128_t here.
    // In LLVM compiler-rt, it performs a 128/128 -> 128 division which is many
    // times slower than necessary. In gcc it's better but still slower than the
    // divlu implementation, perhaps because it's not LIBDIVIDE_INLINEd.
    ...
    // The expression (-shift & 63) is the same as (64 - shift), except it
    // avoids the UB of shifting by 64. The funny bitwise 'and' ensures that
    // numlo does not get shifted into numhi if shift is 0. clang 11 has an x86
    // codegen bug here: see LLVM bug 50118. The sequence below avoids it.
...
static LIBDIVIDE_INLINE void libdivide_u128_shift(uint64_t *u1, uint64_t *u0,
...
static LIBDIVIDE_INLINE struct libdivide_u16_t libdivide_internal_u16_gen(
    ...
    if ((d & (d - 1)) == 0) {
        ...
        proposed_m =
            libdivide_32_div_16_to_16((uint16_t)1 << floor_log_2_d, 0, d, &rem);
...
// The original libdivide_u16_do takes a const pointer. However, this cannot be
// used with a compile time constant libdivide_u16_t: it will generate a warning
// about taking the address of a temporary. Hence this overload.
...
uint16_t libdivide_u16_recover(const struct libdivide_u16_t *denom) {
    ...
    if (!denom->magic) {
        ...
        return 1 + libdivide_32_div_16_to_16(hi_dividend, 0, denom->magic,
                                             &rem_ignored);
    } else {
...
uint16_t libdivide_u16_branchfree_recover(
    const struct libdivide_u16_branchfree_t *denom) {
    uint8_t more = denom->more;
    uint8_t shift = more & LIBDIVIDE_16_SHIFT_MASK;
...
static LIBDIVIDE_INLINE struct libdivide_u32_t libdivide_internal_u32_gen(
    ...
    if ((d & (d - 1)) == 0) {
        ...
        proposed_m =
            libdivide_64_div_32_to_32((uint32_t)1 << floor_log_2_d, 0, d, &rem);
...
uint32_t libdivide_u32_branchfree_recover(
    const struct libdivide_u32_branchfree_t *denom) {
    uint8_t more = denom->more;
    uint8_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
...
static LIBDIVIDE_INLINE struct libdivide_u64_t libdivide_internal_u64_gen(
    ...
    if ((d & (d - 1)) == 0) {
        ...
        proposed_m = libdivide_128_div_64_to_64((uint64_t)1 << floor_log_2_d, 0,
                                                d, &rem);
...
uint64_t libdivide_u64_recover(const struct libdivide_u64_t *denom) {
    ...
    if (!denom->magic) {
        ...
        uint64_t r_hi, r_lo;
        uint64_t half_q = libdivide_128_div_128_to_64(half_n_hi, half_n_lo,
                                                      d_hi, d_lo, &r_hi, &r_lo);
...
static LIBDIVIDE_INLINE struct libdivide_s16_t libdivide_internal_s16_gen(
    ...
    if ((absD & (absD - 1)) == 0) {
        ...
        result.magic = 0;
        result.more =
            (uint8_t)(floor_log_2_d | (d < 0 ? LIBDIVIDE_NEGATIVE_DIVISOR : 0));
    } else {
        ...
        uint16_t rem, proposed_m;
        proposed_m = libdivide_32_div_16_to_16(
            (uint16_t)1 << (floor_log_2_d - 1), 0, absD, &rem);
...
// The original libdivide_s16_do takes a const pointer. However, this cannot be
// used with a compile time constant libdivide_s16_t: it will generate a warning
// about taking the address of a temporary. Hence this overload.
...
int16_t libdivide_s16_branchfree_do(
    int16_t numer, const struct libdivide_s16_branchfree_t *denom) {
    uint8_t more = denom->more;
    uint8_t shift = more & LIBDIVIDE_16_SHIFT_MASK;
    // must be arithmetic shift and then sign extend
...
int16_t libdivide_s16_recover(const struct libdivide_s16_t *denom) {
    ...
    if (!denom->magic) {
        ...
        int magic_was_negated =
            (more & LIBDIVIDE_ADD_MARKER) ? denom->magic > 0 : denom->magic < 0;
        ...
        uint16_t d =
            (uint16_t)(magic_was_negated ? -denom->magic : denom->magic);
        uint32_t n = (uint32_t)1
                     << (16 + shift);  // this shift cannot exceed 30
...
int16_t libdivide_s16_branchfree_recover(
    const struct libdivide_s16_branchfree_t *denom) {
...
static LIBDIVIDE_INLINE struct libdivide_s32_t libdivide_internal_s32_gen(
    ...
    if ((absD & (absD - 1)) == 0) {
        ...
        proposed_m = libdivide_64_div_32_to_32(
            (uint32_t)1 << (floor_log_2_d - 1), 0, absD, &rem);
...
int32_t libdivide_s32_branchfree_do(
    int32_t numer, const struct libdivide_s32_branchfree_t *denom) {
    uint8_t more = denom->more;
    uint8_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
    // must be arithmetic shift and then sign extend
...
int32_t libdivide_s32_recover(const struct libdivide_s32_t *denom) {
    ...
    if (!denom->magic) {
        ...
        uint64_t n = (uint64_t)1
                     << (32 + shift);  // this shift cannot exceed 30
...
int32_t libdivide_s32_branchfree_recover(
    const struct libdivide_s32_branchfree_t *denom) {
...
static LIBDIVIDE_INLINE struct libdivide_s64_t libdivide_internal_s64_gen(
    ...
    if ((absD & (absD - 1)) == 0) {
        ...
        proposed_m = libdivide_128_div_64_to_64(
            (uint64_t)1 << (floor_log_2_d - 1), 0, absD, &rem);
...
int64_t libdivide_s64_branchfree_do(
    int64_t numer, const struct libdivide_s64_branchfree_t *denom) {
    uint8_t more = denom->more;
    uint8_t shift = more & LIBDIVIDE_64_SHIFT_MASK;
    // must be arithmetic shift and then sign extend
...
int64_t libdivide_s64_branchfree_recover(
    const struct libdivide_s64_branchfree_t *denom) {
...
static LIBDIVIDE_INLINE uint16x8_t
libdivide_u16_do_vec128(uint16x8_t numers, const struct libdivide_u16_t *denom);
static LIBDIVIDE_INLINE int16x8_t
libdivide_s16_do_vec128(int16x8_t numers, const struct libdivide_s16_t *denom);
static LIBDIVIDE_INLINE uint32x4_t
libdivide_u32_do_vec128(uint32x4_t numers, const struct libdivide_u32_t *denom);
static LIBDIVIDE_INLINE int32x4_t
libdivide_s32_do_vec128(int32x4_t numers, const struct libdivide_s32_t *denom);
static LIBDIVIDE_INLINE uint64x2_t
libdivide_u64_do_vec128(uint64x2_t numers, const struct libdivide_u64_t *denom);
static LIBDIVIDE_INLINE int64x2_t
libdivide_s64_do_vec128(int64x2_t numers, const struct libdivide_s64_t *denom);
...
static LIBDIVIDE_INLINE uint32x4_t libdivide_u32_neon_srl(uint32x4_t v,
                                                          uint8_t amt) {
    int32_t wamt = (int32_t)(amt);
    return vshlq_u32(v, vdupq_n_s32(-wamt));
}

static LIBDIVIDE_INLINE uint64x2_t libdivide_u64_neon_srl(uint64x2_t v,
                                                          uint8_t amt) {
...
static LIBDIVIDE_INLINE int32x4_t libdivide_s32_neon_sra(int32x4_t v,
                                                         uint8_t amt) {
    int32_t wamt = (int32_t)(amt);
    return vshlq_s32(v, vdupq_n_s32(-wamt));
}

static LIBDIVIDE_INLINE int64x2_t libdivide_s64_neon_sra(int64x2_t v,
                                                         uint8_t amt) {
    int64_t wamt = (int64_t)(amt);
...
static LIBDIVIDE_INLINE uint32x4_t libdivide_mullhi_u32_vec128(uint32x4_t a,
                                                               uint32_t b) {
    // Desire is [x0, x1, x2, x3]
    uint32x4_t w1 = vreinterpretq_u32_u64(
        vmull_n_u32(vget_low_u32(a), b));  // [_, x0, _, x1]
    uint32x4_t w2 =
        vreinterpretq_u32_u64(vmull_high_n_u32(a, b));  //[_, x2, _, x3]
    return vuzp2q_u32(w1, w2);                          // [x0, x1, x2, x3]
}

static LIBDIVIDE_INLINE int32x4_t libdivide_mullhi_s32_vec128(int32x4_t a,
                                                              int32_t b) {
    int32x4_t w1 = vreinterpretq_s32_s64(
        vmull_n_s32(vget_low_s32(a), b));  // [_, x0, _, x1]
    int32x4_t w2 =
        vreinterpretq_s32_s64(vmull_high_n_s32(a, b));  //[_, x2, _, x3]
    return vuzp2q_s32(w1, w2);                          // [x0, x1, x2, x3]
}

static LIBDIVIDE_INLINE uint64x2_t libdivide_mullhi_u64_vec128(uint64x2_t x,
                                                               uint64_t sy) {
    ...
    uint64x2_t temp_lo =
        vshrq_n_u64(vshlq_n_u64(temp, 32), 32);  // temp_lo = temp & 0xFFFFFFFF;
...
static LIBDIVIDE_INLINE int64x2_t libdivide_mullhi_s64_vec128(int64x2_t x,
                                                              int64_t sy) {
...
uint16x8_t libdivide_u16_do_vec128(uint16x8_t numers,
                                   const struct libdivide_u16_t *denom){
...
uint32x4_t libdivide_u32_do_vec128(uint32x4_t numers,
...
uint64x2_t libdivide_u64_do_vec128(uint64x2_t numers,
...
int16x8_t libdivide_s16_do_vec128(int16x8_t numers,
                                  const struct libdivide_s16_t *denom){
...
int32x4_t libdivide_s32_do_vec128(int32x4_t numers,
    ...
    if (!denom->magic) {
        ...
        int32x4_t q = vaddq_s32(
            numers, vandq_s32(vshrq_n_s32(numers, 31), roundToZeroTweak));
        ...
        q = vaddq_s32(
            q, vreinterpretq_s32_u32(vshrq_n_u32(vreinterpretq_u32_s32(q),
                                                 31)));  // q += (q < 0)
        return q;
    }
}
...
int64x2_t libdivide_s64_do_vec128(int64x2_t numers,
    ...
    if (magic == 0) {  // shift path
        uint8_t shift = more & LIBDIVIDE_64_SHIFT_MASK;
        uint64_t mask = ((uint64_t)1 << shift) - 1;
        int64x2_t roundToZeroTweak =
            vdupq_n_s64(mask);  // TODO: no need to sign extend
        // q = numer + ((numer >> 63) & roundToZeroTweak);
        int64x2_t q = vaddq_s64(
            numers,
            vandq_s64(libdivide_s64_signbits(numers), roundToZeroTweak));
        ...
        if (more & LIBDIVIDE_ADD_MARKER) {
            // must be arithmetic shift
            int64x2_t sign =
                vdupq_n_s64((int8_t)more >> 7);  // TODO: no need to widen
            // q += ((numer ^ sign) - sign);
        ...
        q = vaddq_s64(
            q, vreinterpretq_s64_u64(vshrq_n_u64(vreinterpretq_u64_s64(q),
                                                 63)));  // q += (q < 0)
        return q;
    }
}
...
static LIBDIVIDE_INLINE __m512i
libdivide_u16_do_vec512(__m512i numers, const struct libdivide_u16_t *denom);
static LIBDIVIDE_INLINE __m512i
libdivide_s16_do_vec512(__m512i numers, const struct libdivide_s16_t *denom);
static LIBDIVIDE_INLINE __m512i
libdivide_u32_do_vec512(__m512i numers, const struct libdivide_u32_t *denom);
static LIBDIVIDE_INLINE __m512i
libdivide_s32_do_vec512(__m512i numers, const struct libdivide_s32_t *denom);
static LIBDIVIDE_INLINE __m512i
libdivide_u64_do_vec512(__m512i numers, const struct libdivide_u64_t *denom);
static LIBDIVIDE_INLINE __m512i
libdivide_s64_do_vec512(__m512i numers, const struct libdivide_s64_t *denom);
...
static LIBDIVIDE_INLINE __m512i libdivide_s64_shift_right_vec512(__m512i v,
                                                                 int amt) {
    return _mm512_srai_epi64(v, amt);
}

// Here, b is assumed to contain one 32-bit value repeated.
static LIBDIVIDE_INLINE __m512i libdivide_mullhi_u32_vec512(__m512i a,
                                                            __m512i b) {
...
static LIBDIVIDE_INLINE __m512i libdivide_mullhi_s32_vec512(__m512i a,
                                                            __m512i b) {
...
static LIBDIVIDE_INLINE __m512i libdivide_mullhi_u64_vec512(__m512i x,
                                                            __m512i y) {
    ...
    __m512i x1 =
        _mm512_shuffle_epi32(x, (_MM_PERM_ENUM)_MM_SHUFFLE(3, 3, 1, 1));
    __m512i y1 =
        _mm512_shuffle_epi32(y, (_MM_PERM_ENUM)_MM_SHUFFLE(3, 3, 1, 1));
...
static LIBDIVIDE_INLINE __m512i libdivide_mullhi_s64_vec512(__m512i x,
                                                            __m512i y) {
...
__m512i libdivide_u16_do_vec512(__m512i numers,
                                const struct libdivide_u16_t *denom){
...
__m512i libdivide_u32_do_vec512(__m512i numers,
                                const struct libdivide_u32_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        return _mm512_srli_epi32(numers, more);
    } else {
        __m512i q = libdivide_mullhi_u32_vec512(
...
__m512i libdivide_u32_branchfree_do_vec512(
    __m512i numers, const struct libdivide_u32_branchfree_t *denom) {
    __m512i q =
        libdivide_mullhi_u32_vec512(numers, _mm512_set1_epi32(denom->magic));
    __m512i t =
        _mm512_add_epi32(_mm512_srli_epi32(_mm512_sub_epi32(numers, q), 1), q);
    return _mm512_srli_epi32(t, denom->more);
}

////////// UINT64

__m512i libdivide_u64_do_vec512(__m512i numers,
                                const struct libdivide_u64_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        return _mm512_srli_epi64(numers, more);
    } else {
        __m512i q = libdivide_mullhi_u64_vec512(
...
__m512i libdivide_u64_branchfree_do_vec512(
    __m512i numers, const struct libdivide_u64_branchfree_t *denom) {
    __m512i q =
        libdivide_mullhi_u64_vec512(numers, _mm512_set1_epi64(denom->magic));
    __m512i t =
        _mm512_add_epi64(_mm512_srli_epi64(_mm512_sub_epi64(numers, q), 1), q);
    return _mm512_srli_epi64(t, denom->more);
}

////////// SINT16

__m512i libdivide_s16_do_vec512(__m512i numers,
                                const struct libdivide_s16_t *denom){
...
__m512i libdivide_s32_do_vec512(__m512i numers,
                                const struct libdivide_s32_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        uint32_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
        ...
        __m512i q = _mm512_add_epi32(
            numers,
            _mm512_and_si512(_mm512_srai_epi32(numers, 31), roundToZeroTweak));
        ...
        __m512i q = libdivide_mullhi_s32_vec512(
            numers, _mm512_set1_epi32(denom->magic));
        if (more & LIBDIVIDE_ADD_MARKER) {
            // must be arithmetic shift
            __m512i sign = _mm512_set1_epi32((int8_t)more >> 7);
...
__m512i libdivide_s64_do_vec512(__m512i numers,
                                const struct libdivide_s64_t *denom) {
    uint8_t more = denom->more;
    int64_t magic = denom->magic;
    if (magic == 0) {  // shift path
        ...
        __m512i q = _mm512_add_epi64(
            numers, _mm512_and_si512(libdivide_s64_signbits_vec512(numers),
                                     roundToZeroTweak));
        ...
        __m512i q =
            libdivide_mullhi_s64_vec512(numers, _mm512_set1_epi64(magic));
        if (more & LIBDIVIDE_ADD_MARKER) {
            // must be arithmetic shift
            __m512i sign = _mm512_set1_epi32((int8_t)more >> 7);
            // q += ((numer ^ sign) - sign);
...
static LIBDIVIDE_INLINE __m256i
libdivide_u16_do_vec256(__m256i numers, const struct libdivide_u16_t *denom);
static LIBDIVIDE_INLINE __m256i
libdivide_s16_do_vec256(__m256i numers, const struct libdivide_s16_t *denom);
static LIBDIVIDE_INLINE __m256i
libdivide_u32_do_vec256(__m256i numers, const struct libdivide_u32_t *denom);
static LIBDIVIDE_INLINE __m256i
libdivide_s32_do_vec256(__m256i numers, const struct libdivide_s32_t *denom);
static LIBDIVIDE_INLINE __m256i
libdivide_u64_do_vec256(__m256i numers, const struct libdivide_u64_t *denom);
static LIBDIVIDE_INLINE __m256i
libdivide_s64_do_vec256(__m256i numers, const struct libdivide_s64_t *denom);
...
static LIBDIVIDE_INLINE __m256i libdivide_s64_shift_right_vec256(__m256i v,
                                                                 int amt) {
    const int b = 64 - amt;
...
// Here, b is assumed to contain one 32-bit value repeated.
static LIBDIVIDE_INLINE __m256i libdivide_mullhi_u32_vec256(__m256i a,
                                                            __m256i b) {
...
// b is one 32-bit value repeated.
static LIBDIVIDE_INLINE __m256i libdivide_mullhi_s32_vec256(__m256i a,
                                                            __m256i b) {
...
// Here, y is assumed to contain one 64-bit value repeated.
static LIBDIVIDE_INLINE __m256i libdivide_mullhi_u64_vec256(__m256i x,
                                                            __m256i y) {
    // see m128i variant for comments.
...
// y is one 64-bit value repeated.
static LIBDIVIDE_INLINE __m256i libdivide_mullhi_s64_vec256(__m256i x,
                                                            __m256i y) {
...
__m256i libdivide_u16_do_vec256(__m256i numers,
                                const struct libdivide_u16_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        ...
        if (more & LIBDIVIDE_ADD_MARKER) {
            __m256i t = _mm256_adds_epu16(
                _mm256_srli_epi16(_mm256_subs_epu16(numers, q), 1), q);
...
__m256i libdivide_u32_do_vec256(__m256i numers,
                                const struct libdivide_u32_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        return _mm256_srli_epi32(numers, more);
    } else {
        __m256i q = libdivide_mullhi_u32_vec256(
...
__m256i libdivide_u32_branchfree_do_vec256(
    __m256i numers, const struct libdivide_u32_branchfree_t *denom) {
    __m256i q =
        libdivide_mullhi_u32_vec256(numers, _mm256_set1_epi32(denom->magic));
    __m256i t =
...
__m256i libdivide_u64_do_vec256(__m256i numers,
                                const struct libdivide_u64_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        return _mm256_srli_epi64(numers, more);
    } else {
        __m256i q = libdivide_mullhi_u64_vec256(
...
__m256i libdivide_u64_branchfree_do_vec256(
    __m256i numers, const struct libdivide_u64_branchfree_t *denom) {
    __m256i q =
        libdivide_mullhi_u64_vec256(numers, _mm256_set1_epi64x(denom->magic));
    __m256i t =
...
__m256i libdivide_s16_do_vec256(__m256i numers,
                                const struct libdivide_s16_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        ...
        __m256i q = _mm256_add_epi16(
            numers,
            _mm256_and_si256(_mm256_srai_epi16(numers, 15), roundToZeroTweak));
        ...
        if (more & LIBDIVIDE_ADD_MARKER) {
            ...
            // q += ((numer ^ sign) - sign);
            q = _mm256_add_epi16(
                q, _mm256_sub_epi16(_mm256_xor_si256(numers, sign), sign));
        }
        // q >>= shift
...
__m256i libdivide_s16_branchfree_do_vec256(
    ...
    q = _mm256_sub_epi16(_mm256_xor_si256(q, sign),
                         sign);  // q = (q ^ sign) - sign
    return q;
}
...
__m256i libdivide_s32_do_vec256(__m256i numers,
                                const struct libdivide_s32_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        uint32_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
        ...
        __m256i q = _mm256_add_epi32(
            numers,
            _mm256_and_si256(_mm256_srai_epi32(numers, 31), roundToZeroTweak));
        ...
        __m256i q = libdivide_mullhi_s32_vec256(
            numers, _mm256_set1_epi32(denom->magic));
        if (more & LIBDIVIDE_ADD_MARKER) {
            // must be arithmetic shift
            __m256i sign = _mm256_set1_epi32((int8_t)more >> 7);
...
__m256i libdivide_s64_do_vec256(__m256i numers,
                                const struct libdivide_s64_t *denom) {
    uint8_t more = denom->more;
    int64_t magic = denom->magic;
    if (magic == 0) {  // shift path
        ...
        __m256i q = _mm256_add_epi64(
            numers, _mm256_and_si256(libdivide_s64_signbits_vec256(numers),
                                     roundToZeroTweak));
        ...
        __m256i q =
            libdivide_mullhi_s64_vec256(numers, _mm256_set1_epi64x(magic));
        if (more & LIBDIVIDE_ADD_MARKER) {
            // must be arithmetic shift
            __m256i sign = _mm256_set1_epi32((int8_t)more >> 7);
            // q += ((numer ^ sign) - sign);
...
static LIBDIVIDE_INLINE __m128i
libdivide_u16_do_vec128(__m128i numers, const struct libdivide_u16_t *denom);
static LIBDIVIDE_INLINE __m128i
libdivide_s16_do_vec128(__m128i numers, const struct libdivide_s16_t *denom);
static LIBDIVIDE_INLINE __m128i
libdivide_u32_do_vec128(__m128i numers, const struct libdivide_u32_t *denom);
static LIBDIVIDE_INLINE __m128i
libdivide_s32_do_vec128(__m128i numers, const struct libdivide_s32_t *denom);
static LIBDIVIDE_INLINE __m128i
libdivide_u64_do_vec128(__m128i numers, const struct libdivide_u64_t *denom);
static LIBDIVIDE_INLINE __m128i
libdivide_s64_do_vec128(__m128i numers, const struct libdivide_s64_t *denom);
...
static LIBDIVIDE_INLINE __m128i libdivide_s64_shift_right_vec128(__m128i v,
                                                                 int amt) {
    const int b = 64 - amt;
...
// Here, b is assumed to contain one 32-bit value repeated.
static LIBDIVIDE_INLINE __m128i libdivide_mullhi_u32_vec128(__m128i a,
                                                            __m128i b) {
...
// repeated four times.
static LIBDIVIDE_INLINE __m128i libdivide_mullhi_s32_vec128(__m128i a,
                                                            __m128i b) {
...
// Here, y is assumed to contain one 64-bit value repeated.
static LIBDIVIDE_INLINE __m128i libdivide_mullhi_u64_vec128(__m128i x,
                                                            __m128i y) {
    // full 128 bits product is:
    // x0*y0 + (x0*y1 << 32) + (x1*y0 << 32) + (x1*y1 << 64)
    // Note x0,y0,x1,y1 are all conceptually uint32, products are 32x32->64.
...
// y is one 64-bit value repeated.
static LIBDIVIDE_INLINE __m128i libdivide_mullhi_s64_vec128(__m128i x,
                                                            __m128i y) {
...
__m128i libdivide_u16_do_vec128(__m128i numers,
                                const struct libdivide_u16_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        ...
        if (more & LIBDIVIDE_ADD_MARKER) {
            __m128i t =
                _mm_adds_epu16(_mm_srli_epi16(_mm_subs_epu16(numers, q), 1), q);
...
__m128i libdivide_u32_do_vec128(__m128i numers,
                                const struct libdivide_u32_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        ...
        __m128i q =
            libdivide_mullhi_u32_vec128(numers, _mm_set1_epi32(denom->magic));
        if (more & LIBDIVIDE_ADD_MARKER) {
            // uint32_t t = ((numer - q) >> 1) + q;
            // return t >> denom->shift;
            uint32_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
...
__m128i libdivide_u64_do_vec128(__m128i numers,
                                const struct libdivide_u64_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        ...
        __m128i q =
            libdivide_mullhi_u64_vec128(numers, _mm_set1_epi64x(denom->magic));
        if (more & LIBDIVIDE_ADD_MARKER) {
            // uint32_t t = ((numer - q) >> 1) + q;
            // return t >> denom->shift;
            uint32_t shift = more & LIBDIVIDE_64_SHIFT_MASK;
...
__m128i libdivide_s16_do_vec128(__m128i numers,
                                const struct libdivide_s16_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        uint16_t shift = more & LIBDIVIDE_16_SHIFT_MASK;
        ...
        __m128i q = _mm_add_epi16(
            numers,
            _mm_and_si128(_mm_srai_epi16(numers, 15), roundToZeroTweak));
        ...
        if (more & LIBDIVIDE_ADD_MARKER) {
            ...
            // q += ((numer ^ sign) - sign);
            q = _mm_add_epi16(q,
                              _mm_sub_epi16(_mm_xor_si128(numers, sign), sign));
        }
        // q >>= shift
...
__m128i libdivide_s32_do_vec128(__m128i numers,
                                const struct libdivide_s32_t *denom) {
    uint8_t more = denom->more;
    if (!denom->magic) {
        uint32_t shift = more & LIBDIVIDE_32_SHIFT_MASK;
        ...
        __m128i q =
            libdivide_mullhi_s32_vec128(numers, _mm_set1_epi32(denom->magic));
        if (more & LIBDIVIDE_ADD_MARKER) {
            // must be arithmetic shift
            __m128i sign = _mm_set1_epi32((int8_t)more >> 7);
            // q += ((numer ^ sign) - sign);
            q = _mm_add_epi32(q,
                              _mm_sub_epi32(_mm_xor_si128(numers, sign), sign));
        }
        // q >>= shift
...
__m128i libdivide_s64_do_vec128(__m128i numers,
                                const struct libdivide_s64_t *denom) {
    uint8_t more = denom->more;
    int64_t magic = denom->magic;
    if (magic == 0) {  // shift path
        ...
        __m128i q = _mm_add_epi64(
            numers, _mm_and_si128(libdivide_s64_signbits_vec128(numers),
                                  roundToZeroTweak));
        ...
        if (more & LIBDIVIDE_ADD_MARKER) {
            ...
            // q += ((numer ^ sign) - sign);
            q = _mm_add_epi64(q,
                              _mm_sub_epi64(_mm_xor_si128(numers, sign), sign));
        }
        // q >>= denom->mult_path.shift
...
struct NeonVecFor {
    ...
    typedef typename NeonVec<
        sizeof(T) * 8, (((T)0 >> 0) > (T)(-1) ? SIGNED : UNSIGNED)>::type type;
};
...
#define DISPATCHER_GEN(T, ALGO)                                            \
    ...
    LIBDIVIDE_INLINE T recover() const {                                   \
...
class divider {
   private:
    ...
    // Also throw in a shift by 0, which prevents floating point types from
    // being passed.
   ...
   public:
    ...
    // Vector variants treat the input as packed integer values with the same
    // type as the divider (e.g. s32, u32, s64, u64) and divides each of them by
    // the divider, returning the packed quotients.
#if defined(LIBDIVIDE_SSE2)
...
#endif
#if defined(LIBDIVIDE_NEON)
    LIBDIVIDE_INLINE typename NeonVecFor<T>::type divide(
        typename NeonVecFor<T>::type n) const {
        return div.divide(n);
    }
#endif
