...
AngularLsh::AngularLsh(size_t vectorLen, size_t numHashPerBand, size_t numBands)
    : mVectorLen(vectorLen),
      mNumHashPerBand(numHashPerBand),
      mNumBands(numBands) {}

void AngularLsh::generate(uint64_t seed) {
...
vector<uint64_t> AngularLsh::apply(const vector<double> vec, uint64_t seed) {
    vector<uint64_t> res;
...
vector<uint64_t> AngularLsh::apply(const SparseVector& vec, uint64_t seed) {
    vector<uint64_t> res;
    for (size_t i = 0; i < mNumBands; i++) {
        string hashIn;
        for (size_t j = 0; j < mNumHashPerBand; j++) {
...
vector<vector<uint64_t>> AngularLsh::apply(const vector<vector<double>> mat,
                                           uint64_t seed) {
...
double AngularLsh::angular(const vector<double>& vec1,
    ...
    norm1 = sqrt(norm1);
    norm2 = sqrt(norm2);
    return 1.0 - (acos(ip / (norm1 * norm2)) / M_PI);
}

double AngularLsh::angular(const SparseVector& vec1, const SparseVector& vec2) {
...
string AngularLsh::projection(const vector<double>& vec1,
                              const vector<double>& vec2) {
    if (vec1.size() != vec2.size()) {
        invalid_argument("vec1.size() != vec2.size()");
    }
...
std::string AngularLsh::projection(const SparseVector& vec1,
