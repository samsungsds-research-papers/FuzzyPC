...
class Document {
   public:
    ...
    T GetCell(const std::string& pColumnName,
              const std::string& pRowName) const {
        const int columnIdx = GetColumnIdx(pColumnName);
        if (columnIdx < 0) {
            throw std::out_of_range("column not found: " + pColumnName);
        }
   ...
   private:
        ...
        for (auto itr = mData.begin(); itr != mData.end(); ++itr) {
            for (auto itc = itr->begin(); itc != itr->end(); ++itc) {
                if (mSeparatorParams.mAutoQuote &&
                    ...
                    ReplaceString(str, quoteCharStr,
                                  quoteCharStr + quoteCharStr);
    ...
    size_t GetDataColumnCount() const {
        const size_t firstDataRow = static_cast<size_t>(
            (mLabelParams.mColumnNameIdx >= 0) ? mLabelParams.mColumnNameIdx
                                               : 0);
    ...
    inline size_t GetDataRowIndex(const size_t pRowIdx) const {
        const size_t firstDataRow =
            static_cast<size_t>((mLabelParams.mColumnNameIdx + 1 >= 0)
                                    ? mLabelParams.mColumnNameIdx + 1
                                    : 0);
        ...
        if ((mLabelParams.mRowNameIdx >= 0) &&
            ...
            for (auto& dataRow : mData) {
                if (static_cast<int>(dataRow.size()) >
                    mLabelParams.mRowNameIdx) {
                    mRowNames[dataRow[static_cast<size_t>(
                        mLabelParams.mRowNameIdx)]] = i++;
                }
            }
        }
    }
